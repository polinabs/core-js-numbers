...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const topPart = x1 * x2 + y1 * y2;
  const downPart1 = Math.sqrt(x1 ** 2 + y1 ** 2);
  const downPart2 = Math.sqrt(x2 ** 2 + y2 ** 2);
  const cosTeta = topPart / (downPart1 * downPart2);
  const cosTetaInRadians = Math.acos(cosTeta);
  return cosTetaInRadians;
}
...
function toNumber(value, def) {
  ...
  return Number.isNaN(convertedNumber) ? def : convertedNumber;
}
...
function getSumOfDigits(num) {
  ...
  const numArray = numToString.split('').map(Number);
  let sumArray = 0;
